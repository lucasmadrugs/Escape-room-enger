...
(() => {
  ...
  function playExplosionSound() {
    ...
    // MASTER GAIN — crank it up
    const masterGain = ctx.createGain();
    masterGain.gain.setValueAtTime(1.0, now);
    masterGain.connect(ctx.destination);

    // Layer 1: Massive low-frequency noise burst (the BOOM)
    const boomLen = ctx.sampleRate * 4;
    const boomBuffer = ctx.createBuffer(2, boomLen, ctx.sampleRate);
    for (let ch = 0; ch < 2; ch++) {
      const data = boomBuffer.getChannelData(ch);
      for (let i = 0; i < boomLen; i++) {
        const t = i / ctx.sampleRate;
        const decay = Math.exp(-t * 1.8);
        const noise = (Math.random() * 2 - 1);
        const rumble = Math.sin(2 * Math.PI * 30 * t) * 0.8 + Math.sin(2 * Math.PI * 55 * t) * 0.5;
        const crackle = Math.sin(2 * Math.PI * 90 * t) * Math.exp(-t * 2) * 0.4;
        // Clipping/distortion for aggression
        let sample = (noise * 0.7 + rumble + crackle) * decay;
        sample = Math.tanh(sample * 2.5); // soft-clip distortion
        data[i] = sample;
      }
    }
    ...
    boomLowpass.frequency.setValueAtTime(1200, now);
    boomLowpass.frequency.exponentialRampToValueAtTime(60, now + 2.5);
    const boomGain = ctx.createGain();
    boomGain.gain.setValueAtTime(1.5, now);
    boomGain.gain.exponentialRampToValueAtTime(0.001, now + 4);
    const boomCompressor = ctx.createDynamicsCompressor();
    boomCompressor.threshold.setValueAtTime(-6, now);
    boomCompressor.ratio.setValueAtTime(20, now);
    boomCompressor.attack.setValueAtTime(0, now);
    boomSource.connect(boomLowpass);
    boomLowpass.connect(boomCompressor);
    boomCompressor.connect(boomGain);
    boomGain.connect(masterGain);
    boomSource.start();

    // Layer 2: Sharp transient crack (initial impact)
    const crackLen = ctx.sampleRate * 0.3;
    const crackBuffer = ctx.createBuffer(2, crackLen, ctx.sampleRate);
    for (let ch = 0; ch < 2; ch++) {
    ...
    crackHighpass.frequency.setValueAtTime(3000, now);
    ...
    // Layer 3: Ultra sub-bass (you FEEL this)
    const subOsc = ctx.createOscillator();
    subOsc.type = 'sine';
    subOsc.frequency.setValueAtTime(50, now);
    subOsc.frequency.exponentialRampToValueAtTime(15, now + 2);
    ...
    // Layer 4: Mid-range debris rattle
    const debrisOsc = ctx.createOscillator();
    debrisOsc.type = 'sawtooth';
    debrisOsc.frequency.setValueAtTime(200, now);
    debrisOsc.frequency.exponentialRampToValueAtTime(40, now + 1.5);
    const debrisGain = ctx.createGain();
    debrisGain.gain.setValueAtTime(0.6, now);
    debrisGain.gain.exponentialRampToValueAtTime(0.001, now + 1.5);
    const debrisDist = ctx.createWaveShaper();
    const curve = new Float32Array(256);
    for (let i = 0; i < 256; i++) {
      const x = (i * 2) / 256 - 1;
      curve[i] = (Math.PI + 200) * x / (Math.PI + 200 * Math.abs(x));
    }
    debrisDist.curve = curve;
    debrisOsc.connect(debrisDist);
    debrisDist.connect(debrisGain);
    debrisGain.connect(masterGain);
    debrisOsc.start();
    debrisOsc.stop(now + 2);

    // Layer 5: Secondary delayed boom (echo/reverb feel)
    setTimeout(() => {
      const echo = ctx.createOscillator();
      echo.type = 'sine';
      echo.frequency.setValueAtTime(35, ctx.currentTime);
      echo.frequency.exponentialRampToValueAtTime(12, ctx.currentTime + 2);
      const echoGain = ctx.createGain();
      echoGain.gain.setValueAtTime(0.8, ctx.currentTime);
      echoGain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 2);
      echo.connect(echoGain);
      echoGain.connect(masterGain);
      echo.start();
      echo.stop(ctx.currentTime + 2.5);
    }, 150);
  }
  ...
  // ---- Audio Button System ----
  // Audio files should be placed in the /audio/ subfolder:
  //   audio/audio1.mp3
  //   audio/audio2.mp3
  //   audio/audio3.mp3
  //   audio/audio4.mp3
  ...
  document.querySelectorAll('.audio-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      audio.play().then(() => {
        ...
        // File not found — visual feedback
